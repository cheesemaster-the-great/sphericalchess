...
let isVRMode = false;
let vrInteractionActive = false;
let vrInteractionStartPoint = new THREE.Vector3();
let vrInteractionStartRotation = new THREE.Quaternion();
    ...
    vrButton.addEventListener('click', async () => {
        if (renderer.xr.isPresenting) {
            ...
            try {
                ...
                vrStatus.textContent = 'In VR Mode - Walk around! 🚶‍♂️';
                isVRMode = true;
...
function onControllerSelectStart(event) {
    ...
    if (intersects.length > 0) {
        vrInteractionActive = true;
        vrInteractionStartPoint.copy(controller.position);
        earthMesh.quaternion.clone(vrInteractionStartRotation);
    }
}
...
function handleVRInteraction() {
    if (!renderer.xr.isPresenting || !vrInteractionActive) return;
    
    // Handle controller-based Earth rotation
    [controller1, controller2].forEach(controller => {
        if (controller && vrInteractionActive) {
            // Calculate rotation based on controller movement
            const delta = new THREE.Vector3().subVectors(
                controller.position, 
                vrInteractionStartPoint
            );
            
            // Apply rotation with damping
            earthMesh.rotation.y = vrInteractionStartRotation.y + delta.x * 2;
            earthMesh.rotation.x = vrInteractionStartRotation.x + delta.y * 2;
